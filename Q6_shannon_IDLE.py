...
while particle_count < max_particles:
    ...
    grids_list.append(grid.copy())


# +
# Draw the particles on the screen
plt.matshow(grids_list[1000], cmap='RdPu')


#plt.colorbar()
plt.show()

#plt.savefig('A2P2_graph')


# +
snapshots = grids_list

# First set up the figure, the axis, and the plot element we want to animate
fig = plt.figure( figsize=(8,8) )

a = snapshots[0]
im = plt.imshow(a, vmin=0, vmax=2, cmap='RdPu', animated=True)
...
#so far run 10 looks the best
anim.save('A2P2_try1.gif', dpi=90)
